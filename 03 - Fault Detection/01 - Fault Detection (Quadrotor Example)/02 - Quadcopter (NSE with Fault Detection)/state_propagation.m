function [X,U] = state_propagation(t,X,control,d,fp)
...
udot     = d(1)/fp.m - km*u1*(1/fp.m)*(cos(psi)*sin(theta)*cos(phi) + sin(psi)*sin(phi));
vdot     = d(2)/fp.m - km*u1*(1/fp.m)*(sin(psi)*sin(theta)*cos(phi) - cos(psi)*sin(phi));
wdot     = d(3)/fp.m + fp.g - km*u1*(1/fp.m)*cos(theta)*cos(phi);
...
rdot     = (fp.Ixx-fp.Iyy)/fp.Izz*p*q + km*u4/fp.Izz;
